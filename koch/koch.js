...
function koch4(n, alpha, x0, y0, length) {
	if (n == 0) {
		var x1 = parseInt(x0, 10);
		var y1 = parseInt(y0, 10);
		var x2 = parseInt((x0 + length * Math.cos(alpha)), 10);
		...
		var x2 = x1 + length * Math.cos(alpha);
		var y2 = y1 + length * Math.sin(alpha);
		var x3 = x2 + length * Math.cos(alpha + toRadians(90));
		var y3 = y2 + length * Math.sin(alpha + toRadians(90));
		var x4 = x3 + length * Math.cos(alpha);
		var y4 = y3 + length * Math.sin(alpha);
		var x5 = x4 + length * Math.cos(alpha - toRadians(90));
		var y5 = y4 + length * Math.sin(alpha - toRadians(90));
		koch4(n-1, alpha, x1, y1, length);
		koch4(n-1, alpha + toRadians(90), x2, y2, length);
...
function update(f) {
	...
	koch3(N, 0, 10, canvas.height / 4, canvas.width - 20);
